...
// Copyright 2011-2013 ThePiachu. All rights reserved.
...
func (b Base58) ToBig() *big.Int {
	...
	for i := 0; i < len(b); i++ {
		...
		answer.Add(answer, big.NewInt(int64(BitcoinRevAlp[string(b[i:i+1])]))) //add value of the current letter
	}
	return answer
}
...
func (b Base58) ToInt() int {
	...
	for i := 0; i < len(b); i++ {
		...
		answer += BitcoinRevAlp[string(b[i:i+1])] //add value of the current letter
	}
	return answer
}
...
func Big2Base58(val *big.Int) Base58 {
	...
	for valCopy.Cmp(big.NewInt(0)) > 0 { //converts the number into base58
		...
		tmpStr += BitcoinAlphabet[tmp.Int64() : tmp.Int64()+1] //encodes
	}
...
func Int2Base58(val int) Base58 {
	...
	for valCopy > 0 { //converts the number into base58
		...
		tmpStr += BitcoinAlphabet[tmp : tmp+1] //encodes
	}
	for i := (len(tmpStr) - 1); i > -1; i-- {
		answer += tmpStr[i : i+1] //reverses the order
	}
...
func Hex2Base58(val []byte) Base58 {
	...
	i := 0
	for i = 0; i < len(val) && val[i] == 0; i++ {
	}
	answer := ""
	for j := 0; j < i; j++ { //adds zeroes from the front
		answer += BitcoinAlphabet[0:1]
	}
